...
unsigned int nStakeMinAge = 60 * 60 * 24 * 30;	// minimum age for coin age: 1d
unsigned int nStakeMaxAge = 60 * 60 * 24 * 90;	// stake age of full weight: 100d
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in OperaCoin
    bool fStrictPayToScriptHash = true; // Always active in OperaCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "OperaCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "September 9, 2016, Arjun Mishra builds operacoin.";
        CTransaction txNew;
